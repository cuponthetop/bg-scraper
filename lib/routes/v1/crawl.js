...
module.exports = function (controllers) {
  ...
  crawler.post('/once/:jobtype', function (req, res) {
    req.checkParams('jobtype', 'Invalid crawler job type').notEmpty().isAlpha();
    req.checkBody('url', 'Invalid additional info url').optional().isURL();
    ...
    req.getValidationResult()
      .then(function (result) {
        ...
        return crawlerCTRL.getJobInfo(req.params.jobtype, additionalData);
      })
      .then((jobInfo) => {
        return crawlerCTRL.crawlOnce(req.params.jobtype, jobInfo.crawlFunc, jobInfo.chainedFunc, jobInfo.args);
      })
      .then((result) => {
  ...
  crawler.post('/job/:jobtype', function (req, res) {
    ...
    req.getValidationResult()
      .then(function (result) {
        ...
        crawlerCTRL.getJobInfo(req.params.jobtype, additionalData)
          .then((jobInfo) => {
